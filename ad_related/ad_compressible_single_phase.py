...
class ArithmeticAverageAd(ApplicableOperator):
    ...
    def apply(self, inner_values, dir_bound_values):
        ...
        bc = d[pp.PARAMETERS][param_key]["bc"].copy()  # Do we need copy (?)
...
for n in range(1, num_time_steps + 1):
    ...
    while iteration_counter <= 30 and not (rel_res < 1e-12 or residual_norm < 1e-12):
        ...
        if avg_method == "arithmetic":
            rho_faces_ad = arithmetic_avg(rho_ad(p), rho_ad(dir_bound_ad))
        ...
        advective_flux_ad = flux_ad * rho_faces_ad  # advective flux
        ...
        continuity_ad = (
            phi * mass_ad.mass * (rho_ad(p) - rho_ad(p.previous_timestep()))
