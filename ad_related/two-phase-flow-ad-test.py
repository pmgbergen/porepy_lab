...
Credit and first contact: Jakub Both.
...
lam_b_old_up = fluxBasedUpwind(
    lam_b(sb.previous_timestep(), sc.previous_timestep()),
    pp.ad.Array(lambda_b(np.zeros_like(bc_val_flow), np.ones_like(bc_val_flow))),
    total_flux,
)
lam_c_old_up = fluxBasedUpwind(
    lam_c(sb.previous_timestep(), sc.previous_timestep()),
    pp.ad.Array(lambda_c(np.zeros_like(bc_val_flow), np.ones_like(bc_val_flow))),
    total_flux,
)

v_res = sb.previous_timestep() + sc.previous_timestep() - pp.ad.Array(one_vector)
total_flow_source = (
    source_c / rho_c + source_b / rho_b + porosity * res_vol_factor * v_res
)
pressure_eq = pp.ad.Expression(
    div * total_flux, dof_manager=dof_manager, name="pressure equation"
)
...
for n in range(1, num_time_steps + 1):
    ...
    d[pp.STATE][brine_saturation_variable] = d[pp.STATE][pp.ITERATE][
        brine_saturation_variable
    ].copy()
    d[pp.STATE][co2_saturation_variable] = d[pp.STATE][pp.ITERATE][
        co2_saturation_variable
    ].copy()
